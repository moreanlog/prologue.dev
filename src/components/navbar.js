...
export default function Navbar(){
  ...
  const scrollDirection = useScrollDirection();
    ...
    return (
      <header  className={`active ${ scrollDirection === "down" ? "-top-24" : "top-0"} py-3 sticky z-10 bg-white dark:bg-black backdrop-filter backdrop-blur-lg bg-opacity-[78.6%] border-b border-zinc-50 dark:border-zinc-800 px-4 transition-all duration-600 transform`}>
          ...
          <div className="hidden sm:block" tabIndex="0">
            {headerNavLinks.map((link) => (
              <Link
                key={link.title}
                href={link.href}
                className="rounded-lg px-3 py-2 font-normal text-zinc-500 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-slate-800 transition trasnform duration-400 select-none"
                 >
                {link.title}
              </Link>
            ))}
